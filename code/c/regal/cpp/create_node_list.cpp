...
int main(int argc, char* argv[])
{
	...
	string network_label; // newman, bio ...
    ...
    string g1_nodelist_file_name; // name of the nodes list file for the first network
    ...
    vector<string> g1_id_name_mapping (g1_size);
    for(auto it = g1_name_id_mapping.begin(); it != g1_name_id_mapping.end(); ++it)
    {
        g1_id_name_mapping[it->second] = it->first;
    }

    // --------- Fill out the g1 nodelist file ---------
    ofstream g1_nodelist_file (g1_nodelist_file_name); // std::ofstream::out
    
    for(int i = 0; i < g1_size; ++i)
    {
        string line = "";
        line = line + g1_id_name_mapping[i] + "\n";
        g1_nodelist_file<<line;
    }
	...
	for(int unique_i = 0; unique_i < vector_p.size(); ++unique_i)
	{
        ...
        vector<string> g2_id_name_mapping (g2_size);
        for(auto it = g2_name_id_mapping.begin(); it != g2_name_id_mapping.end(); ++it)
        {
            g2_id_name_mapping[it->second] = it->first;
        }

        // --------- Fill out the g2 node list file ---------
        ofstream g2_nodelist_file (g2_nodelist_file_name); // std::ofstream::out
        
        for(int ii = 0; ii < g2_size; ++ii)
        {
            string inner_line = "";
            inner_line = inner_line + g2_id_name_mapping[ii] + "\n";

            g2_nodelist_file<<inner_line;
        }
