...
int main(int argc, char* argv[])
{
	...
	(use_seed_alignment_method)? cout<<"Using Seed Method:\n" : cout<<"Using Naive Method:\n";
	...
	for(int unique_i = 0; unique_i < vector_p.size(); ++unique_i)
	{
		...
		if(!use_seed_alignment_method)
		{
			...
			ec_result_vector.push_back(ini_ec_naive);
			s3_result_vector.push_back(ini_s3_naive);
		}

		else //using seed alignment
		{
			...
			ec_result_vector.push_back(ini_ec_seed);
			s3_result_vector.push_back(ini_s3_seed);
		}
