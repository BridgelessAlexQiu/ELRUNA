...
int main(int argc, char* argv[])
{
	...
	for(int unique_i = 0; unique_i < vector_p.size(); ++unique_i)
	{
        ...
        l_file_name = "../L/self_under_noise/" + network_type + "/" + network_label + "/" + network_label + "_" + p + "_l.txt";
        ...
        // -------- Fill out the L file ----------
        ofstream l_file (l_file_name); // std::ofstream::out
        int threshold = (int) log2(g1_size);
        threshold *= 2; // Some nodes are left unaligned, so I purposely increased the threshold

        for(int i2 = 0; i2 < g1_size; ++i2)
        {
            vector<array<double, 2>> diff (g2_size);
            for(int j2 = 0; j2 < g2_size; ++j2)
            {
                diff[j2][0] = (double) j2;
                diff[j2][1] = (double) min(g1_degree_sequence[i2], g2_degree_sequence[j2]) / (double) max(g1_degree_sequence[i2], g2_degree_sequence[j2]);
            }

            sort(diff.begin(), diff.end(), [](const array<double, 2>& a, const array<double, 2>& b) {return a[1] > b[1];});

            for(int t = threshold+1; t < g2_size; ++t)
            {
                diff[t][1] = 0.0;
            }

            vector<double> final_diff (g2_size);

            for(int j3 = 0; j3 < g2_size; ++j3)
            {
                final_diff[(int)diff[j3][0]] = diff[j3][1];
            }
            ...
            for(int j3 = 0; j3 < g2_size; ++j3)
            {
                if(j3 < g2_size - 1)
                {
                    line2 = line2 + to_string(final_diff[j3]) + ",";
                }
                else
                {
                    line2 = line2 + to_string(final_diff[j3]) + "\n";
